...
import { string, number, stringSet } from "@aurios/mizzle/columns";
...
describe("Update Builder", () => {
  ...
  it("should correctly resolve keys and construct SET expression", async () => {
    ...
    await db.update(user).set({ name: "John", age: 30 }).where(eq(user.id, "123")).execute();

    expect(mockDocClient.send).toHaveBeenCalled();
    const call = (mockDocClient.send as any).mock.calls[0][0];
  ...
  it("should correctly construct ADD expression for numbers and sets", async () => {
    ...
    await db.update(user).add({ age: 1 }).where(eq(user.id, "123")).execute();
  ...
  it("should correctly construct REMOVE expression", async () => {
    ...
    await db.update(user).remove("age", "name").where(eq(user.id, "123")).execute();
  ...
  it("should allow explicit key provision via key()", async () => {
    ...
    await db.update(user).set({ name: "John" }).key({ pk: "MANUAL#123", sk: "METADATA" }).execute();
