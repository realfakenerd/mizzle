...
describe("Unified Select Integration", () => {
    ...
    it("should support Get, Query and Scan", async () => {
        ...
        const _u2 = await db.insert(user).values({ name: "Bob", age: 30, status: "active" }).returning().execute();
        const _u3 = await db.insert(user).values({ name: "Charlie", age: 35, status: "inactive" }).returning().execute();
        ...
        const _queryGsiRes = await db.select()
            .from(user)
        ...
        const _filterRes = await db.select()
            .from(user)
            .where(eq(user.status, "active"))
            .execute();
