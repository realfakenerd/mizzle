...
describe('JSDoc Presence for Schema Definition', () => {
    ...
    describe('Core Schema Functions', () => {
        const filePath = path.resolve(__dirname, '../packages/mizzle/src/core/table.ts');
    ...
    describe('Column Builders', () => {
        const columnFiles = [
            'string.ts', 'number.ts', 'boolean.ts', 'date.ts', 'uuid.ts', 'json.ts',
            'binary.ts', 'list.ts', 'map.ts', 'string-set.ts', 'number-set.ts', 'binary-set.ts'
        ];

        columnFiles.forEach(file => {
            const builderName = file.replace('.ts', '').replace(/-([a-z])/g, (_, letter) => letter.toUpperCase()); // camelCase
            
            it(`should have JSDoc for column builder: ${builderName} in ${file}`, () => {
                const filePath = path.resolve(__dirname, `../packages/mizzle/src/columns/${file}`);
                const content = readFileSync(filePath, 'utf-8');
