...
export default defineConfig({
  ...
  test("Full lifecycle: generate -> push -> list -> drop", async () => {
    ...
    const {
      ...
      stdout: genStdout,
      stderr: genStderr,
    } = await runCommand(["packages/mizzling/src/cli.ts", "generate", "--name", "initial"], {
    ...
    const {
      ...
      stdout: pushStdout,
      stderr: pushStderr,
    } = await runCommand(["packages/mizzling/src/cli.ts", "push", "--yes"], {
      MIZZLE_CONFIG: CONFIG_FILE,
    });
    ...
    // 3. List
    const { stdout: listOutput } = await runCommand(["packages/mizzling/src/cli.ts", "list"], {
      MIZZLE_CONFIG: CONFIG_FILE,
    });
