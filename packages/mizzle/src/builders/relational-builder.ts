...
type WhereCallback<T extends Entity> = (fields: T["_"]["columns"], ops: Operators) => Condition;
  ...
  async findMany<TOptions extends RelationalQueryOptions<T>>(
    ...
    if (options.where) {
      const columns = (this.table._?.columns ||
        (this.table as unknown as Record<string, Column>)) as Record<string, Column>;
      if (typeof options.where === "function") {
        condition = (options.where as WhereCallback<T>)(
          columns as unknown as Record<string, Column>,
          operators,
        );
      } else {
    ...
    if (
      ...
      results = parser.parse(
        logicalItems as Record<string, unknown>[],
        this.entityName,
        (options.with || options.include) as Record<string, boolean | object>,
      );
      ...
      await Promise.all(
        results.map(async (result) => {
          for (const [relName, relOptions] of Object.entries(relationsToFetch)) {
            ...
            const hasNestedRelations =
              typeof relOptions === "object" && (relOptions.with || relOptions.include);

            if (isAlreadyPopulated && !hasNestedRelations) continue;

            const targetEntity = relation.config.to;
            const targetEntityName = Object.entries(this.schema!.entities).find(
              ([_, m]) => m.entity === targetEntity,
            )?.[0];
            ...
            if (relation.config.fields && relation.config.references) {
              ...
              relation.config.fields.forEach((fieldCol, idx) => {
                ...
                const targetLogicalEntry = Object.entries(
                  targetEntity[ENTITY_SYMBOLS.COLUMNS] as Record<string, Column>,
                ).find(([_, c]) => c === refCol || c.name === refCol.name);

                const targetLogicalName = targetLogicalEntry?.[0];

                if (targetLogicalName) {
                  const sourceLogicalName = Object.entries(
                    this.table[ENTITY_SYMBOLS.COLUMNS] as Record<string, Column>,
                  ).find(([_, c]) => c === fieldCol || c.name === fieldCol.name)?.[0];

                  const val = sourceLogicalName
                    ? (result[sourceLogicalName] ?? result[fieldCol.name])
                    : result[fieldCol.name];
                  if (val !== undefined) {
            ...
            if (targetRes.hasPartitionKey) {
              const targetQb = new RelationnalQueryBuilder(
                this.client,
                targetEntity,
                this.schema,
                targetEntityName,
