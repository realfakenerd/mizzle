...
import { BinaryExpression, LogicalExpression, type Expression } from "../expressions/operators";
 ...
 */
export function compositeKey(separator: string, ...segments: (string | Column)[]): KeyStrategy {
  return { type: "composite", separator, segments };
}

function extracValuesFromExpression(expression: Expression, acc: Record<string, unknown>): void {
...
function resolveKeyStrategy(
  ...
  if (
    strategy.type === "prefix" &&
    strategy.segments.length === 2 &&
    typeof strategy.segments[0] === "string" &&
    strategy.segments[1] instanceof Column
  ) {
...
export function resolveStrategies(
  ...
  const strategies = entity[ENTITY_SYMBOLS.ENTITY_STRATEGY] as unknown as Record<
    string,
    KeyStrategy | { pk: KeyStrategy; sk?: KeyStrategy }
  >;
  const physicalTable = entity[ENTITY_SYMBOLS.PHYSICAL_TABLE] as unknown as Record<
    string | symbol,
    unknown
  >;
  ...
  if (forcedIndexName) {
    ...
    const indexBuilder = indexes?.[forcedIndexName] as
      | { config: { pk: string; sk?: string } }
      | undefined;
    const strategy = strategies[forcedIndexName];
    const indexStrategy =
      strategy && "pk" in strategy
        ? (strategy as { pk: KeyStrategy; sk?: KeyStrategy })
        : undefined;
    ...
    if (indexBuilder && indexStrategy) {
      ...
      if (indexPkValue) {
        ...
        const indexSkValue = indexStrategy.sk
          ? resolveKeyStrategy(indexStrategy.sk, availableValues)
          : undefined;
        ...
        if (indexStrategy.pk && indexBuilder.config.pk) {
          ...
          if (indexPkValue) {
            ...
            if (indexStrategy.sk && indexBuilder.config.sk) {
              const indexSkValue = resolveKeyStrategy(indexStrategy.sk, availableValues);
              if (indexSkValue) {
                result.keys[indexBuilder.config.sk] = indexSkValue;
                result.hasSortKey = true;
              }
