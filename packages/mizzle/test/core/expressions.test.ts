...
import { buildExpression } from "@aurios/mizzle/expressions/builder";
import {
  eq,
  lt,
  and,
  ...
  beginsWith,
  attributeExists,
} from "@aurios/mizzle/expressions/operators";
import type { Column } from "@aurios/mizzle/core/column";
...
describe("Expression Builder", () => {
  ...
  it("should build function expressions", () => {
    expect(buildExpression(beginsWith(colA, "pre"), addName, addValue)).toBe(
      "begins_with(#col_a, :pre)",
    );
    expect(buildExpression(attributeExists(colB), addName, addValue)).toBe(
      "attribute_exists(#col_b)",
    );
  });
