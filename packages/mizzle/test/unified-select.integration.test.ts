...
import { string, uuid, number } from "@aurios/mizzle/columns";
import { prefixKey, staticKey, gsi } from "@aurios/mizzle";
...
import { eq, lt } from "@aurios/mizzle/expressions/operators";
...
describe("Unified Select Integration", () => {
  ...
  it("should support Get, Query and Scan", async () => {
    ...
    const u1 = await db
      .insert(user)
      .values({ name: "Alice", age: 25, status: "active" })
      .returning()
      .execute();
    const _u2 = await db
      .insert(user)
      .values({ name: "Bob", age: 30, status: "active" })
      .returning()
      .execute();
    const _u3 = await db
      .insert(user)
      .values({ name: "Charlie", age: 35, status: "inactive" })
      .returning()
      .execute();
    ...
    const _queryGsiRes = await db.select().from(user).where(eq(user.status, "active")).execute();

    // This might fail to use GSI automatically if not implemented
    expect(_queryGsiRes.length).toBeGreaterThanOrEqual(2);

    // 3. Query with Filter (Should use FilterExpression)
    // Current SelectBase ignores extra where clauses
    const _filterRes = await db.select().from(user).where(eq(user.status, "active")).execute();
    ...
    expect(filtered.some((u: Record<string, unknown>) => u.name === "Alice")).toBe(true);
    expect(filtered.every((u: Record<string, unknown>) => (u.age as number) < 28)).toBe(true);
