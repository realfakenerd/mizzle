...
interface MinimalEntity {
  [ENTITY_SYMBOLS.COLUMNS]: Record<string, MinimalColumn>;
}

export function mapToLogical(
  ...
  if (item["pk"] && !Object.values(columns).some((c) => c.name === "pk"))
    logicalItem["pk"] = item["pk"];
  if (item["sk"] && !Object.values(columns).some((c) => c.name === "sk"))
    logicalItem["sk"] = item["sk"];
  return logicalItem;
}

export interface ColumnType<TData = unknown, THasDefault extends boolean = boolean> {
  _$data: TData;
